...
- **Solver**: Python serverless functions on Vercel (`/api/python/`), OR-Tools VRP
...
- **Python solver**: `/api/python/optimize.py` (Vercel Python runtime, pinned via `runtime.txt` to 3.12). Don't touch `vercel.json`'s `runtime` field — auto-detect handles it; explicit values like `"python3.12"` are invalid and break deploys.
...
- **Sustainable workload bands** (clock-hrs/crew/week): `<40` over-provisioned · `40–50` sustainable · `50–55` tight · `55–60` add 1–2 crews · `>60` unsustainable. Defined in `api/python/optimize.py::_classify_capacity` and mirrored in `src/app/capacity/page.tsx`.
